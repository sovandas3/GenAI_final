...
# LangGraph + Streamlit version (no Autogen)
#
...
- Rate disruption risk as Low / Medium / High.
...
- Route & date
- Flight JSON
- Flight summary (already in markdown)
- Weather & disruption analysis (already in markdown)
- Live flight count over India
- Boolean flag: map_included

Produce a polished markdown report with this structure:

### 🛫 Route Overview
- Route: <SRC> → <DST>
- Date: <DATE>
- User query: <short paraphrase>

### 📋 Flight Schedule Snapshot
- If flights exist (flight_json['count'] > 0): show a markdown table:
  Airline | Flight | Departure | Arrival
- If none: clearly say schedule data is not available.
- After the table (or message), insert the provided "Flights Summary" markdown.

### 🌦 Weather & Operational Context
- Insert the provided weather/disruption markdown.

### 📡 Live Airspace Status
- Show "Tracked flights over India: N"
- One-line interpretation: light / normal / busy.

### 🗺 Map Visualization
- If map_included is true: write "✅ Live airspace map is displayed below for visual monitoring."
- Else: write "ℹ Map not displayed. Ask for 'live map' or 'tracking' to see it."

### 🎯 Advisor Recommendation
- 2–4 bullet points:
  - Overall disruption risk: Low / Medium / High.
  - Suggested actions (monitor, reroute, rebook, adjust buffers).
  - Any time-window to avoid (e.g., early morning due to fog).
- End with "**Confidence:** XX%".

Do NOT dump JSON; keep it clean and readable.
"""
    user = f"""
...
Flights summary (markdown):
{flight_summary}

Weather + disruption (markdown):
{weather_block}
