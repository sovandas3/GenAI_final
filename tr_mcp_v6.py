...
# Demo-only credentials (email -> password)
USER_CREDENTIALS = {
...
# ---------------- APP / API CONFIG ---------------------------
###############################################################
OPENAI_API_KEY = "sk-_hlWKUmXssdcqD4O7AI3hQ"
...
def tavily_search_disruptions(city: str, src: str, dst: str) -> str:
    """App-side Tavily search for disruptions (used by weather node)."""
...
###############################################################
# -------------------- MCP SERVER (TAVILY ONLY) --------------
###############################################################

mcp = FastMCP("Travel-Disruption-MCP")

@mcp.tool()
def tavily_search_disruptions_mcp(query: str) -> Dict[str, Any]:
    """
    MCP-exposed Tavily search for disruptions.
    This is generic, unlike the app helper which is route-specific.
    """
    url = "https://api.tavily.com/search"
    payload = {
        "api_key": TAVILY_API_KEY,
...
def login_ui():
    st.title("🔐 Login – Travel & Logistics Real-Time Disruption Advisor")
...
def run_streamlit_app():
    ...
    st.sidebar.markdown(f"👤 Logged in as: **{st.session_state.get('user_email', 'Unknown')}**")
...
if __name__ == "__main__":
    # If called as: python travelAdvisor.py mcp  -> run MCP server
    # Otherwise, just run Streamlit app (if you do: streamlit run travelAdvisor.py,
    # Streamlit will import this file and execute run_streamlit_app via top-level).
    if len(sys.argv) > 1 and sys.argv[1].lower() == "mcp":
        start_mcp_server()
    else:
        # For direct python execution (not via streamlit)
        run_streamlit_app()
